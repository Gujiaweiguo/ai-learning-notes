...
## 2026-09-23（W17-D3 周三 · MCP 工具描述 Change 起草 t01 × 工具宇宙漂移取证 × Skills 陈旧引用 26 天破案）

### 今天最大的认知
Today's Question「工具描述是给谁看的资产——给人还是给 AI？为什么它需要治理头」的答案：**机器消费的资产腐烂时没有人疼**。tools/list 的 description 是 LLM 选型的唯一菜单、skills 的 required/optional 是数字员工的课程表——这份契约烂了没有编译错误、没有测试失败、没有工单，只有静默降级（技能走不通→数字员工哑火→没人知道为什么）。破案实证：skills 4 件中 2 件引用已改名/已移除的工具，**4 处漂移、3 处在 required 位置、26 天无人发现**（skills 冻结 2026-06-15 / 改名 2026-08-28 fbacc0f1 / 今日取证）。最反直觉的是：主仓并非没有治理意识——toolPermissions 层 canonical catalog 对齐 + auth_mapping_test 双向覆盖测试 + spec 归档一应俱全，**同一个工具面，权限层有门、语义层裸奔**。治理盲区不在技术，在所有权：权限是安全资产（有人负责），描述/技能引用是语义资产（无人认领）。ERP 映射：老 ERP 里「功能权限表」从来有人管，但「操作手册」从来没人管——因为手册是给人看的，人看不懂会问；AI 消费的操作手册没人替它问。

### 今天最大的坑
提案首版的工具按文件分布写了 8/11/10（肉眼数 grep 命中行），脚本提取实数 9/10/10——**进提案的每个数字必须过脚本，不能靠肉眼数 grep**（grep 行号里有 ActorName 之类的非 RegisterTool 命中）。幸好在写 ipynb 断言时对账发现，提案修正后落盘。次小坑：matplotlib 标注用 ✗（U+2717）NotoSansCJK 无此字形告警，换 ×（U+00D7）干净——字形集先验在 D2 就有，今天又验证一次。

### 今天最大的决策
① **t01 提案 draft 落盘**（定轨第 6 项，w14-plus W16+ 方向 2「第一份 AI 侧 change」）：changes/_drafts/t01-mcp-tool-semantic-description-governance.md，Why 四链全部今日实考（skills 漂移活事件 / 描述无治理资产 29 条英文 36-172ch 仅 1 含中文 / 表宇宙有门工具宇宙没有 / 权限映射模式可平移），What 四条（mcp-tool-registry.yaml 29 条登记 / skills 引用覆盖门照抄 auth_mapping_test 双向模式且首个动作即修 4 漂移 / 描述双语化 v2 生成器在语义层·生成不豁免裁决 / Go↔registry 描述 diff），验收线 4 条可度量；OQ-1 registry 归属倾向 lnkcre 仓内（26 天漂移根因=声明资产离执行面太远），升格立案待周日 Review。② **后置条件核验在案**：probe 实跑 origin 集合指纹 7995cb4839ddc5b1 = w40 基线（30 commits 零新表）+ cited 22/22——g0x Impact 段「MCP 工具描述生成获得表宇宙稳定性判定依据」的预言兑现件。③ **v1 只校验不生成**（OQ-2）：先证明存量对齐（29/29 登记、4/4 修漂移、改名演练 RED）再谈双语化生成——与 G-05 v0.2 顺序律同构（先零误伤，再新增检出）。

### 数字速览
工具宇宙 29（tools.go 9 / extended 10 / workflow_definition 10）与 toolPermissions 29 条集合相等（断言）｜描述画像 36-172ch 中位 102，含中文 1/29｜skills 漂移 4 处（required ×3）：mi-quotation（generate_document 已移除+convert_quotation_to_contract 从未注册）、mi-service-triage（create_service_request/query_tickets 已改名）｜漂移年龄 26 天（06-15→08-28→09-23）｜SoT 术语直接对齐 0/80 别名｜覆盖门模拟：改名回放 GREEN→RED 当次检出（对照无门 26 天）｜probe：lnkcre behind 30（表宇宙零漂移）/ docs 3 / LnkChatBI 18（**首次出现主仓侧活动，周一 W40 digest 议题**）｜canonical 门 GREEN ｜ ipynb verify OK（7 code cells，6 组断言，3 图）
