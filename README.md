...
- **第一周**（6/2-6/8）：Transformer 深入理解
  - Day1（6/4 周四）= 自注意力机制 ✅
  - Day2（6/5 周五）= Multi-Head Attention ✅
- **第二周**（6/9-6/15）：Transformer 深入理解（续）
  - Day1（6/9 周一）= FFN、LayerNorm 与残差连接
  - Day2（6/10 周二）= Tokenizer 与词嵌入
  - Day3（6/11 周三）= 整体架构回顾（GPT vs BERT）
  - Day4（6/12 周四）= ⚡代码实战（NumPy实现Self-Attention）
  - Day5（6/13 周五）= 🔄复习日
- **第三周**（6/16-6/22）：大模型训练全景
  - Day1 = 预训练：数据、规模与涌现能力
  - Day2 = SFT：监督微调
  - Day3 = RLHF：人类反馈强化学习
  - Day4 = DPO 与对齐方法对比
